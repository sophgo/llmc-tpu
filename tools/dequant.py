    ...
    for i in range(block_num):
        for layer_name in Qwen2_5["block"]:
            ...
            if isinstance(q_layer,WQLinear_GEMM):
                iweight = dequantize_gemm(q_layer.qweight, q_layer.qzeros, q_layer.scales, q_layer.w_bit, q_layer.group_size)
                iweight = iweight.T
                ...
                print(f"Updated layer {i}.{layer_name}.weight")
                
                if hasattr(q_layer, 'bias') and q_layer.bias is not None:
                    q_layer.bias = q_layer.bias.to(device=p_layer.bias.device, dtype=p_layer.bias.dtype)
